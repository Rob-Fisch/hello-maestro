...
| 3 | **Stage Plot (Public Event Sharing)** | Yes | Yes | Yes | Share a beautiful public event page with fans - include setlists, bio, tip jar, and mailing list signup. | Public event pages at `/fan/{eventId}`. | Available to all tiers. Future: QR codes. | `events.is_public_stage_plot`, `events.public_description`, `events.show_setlist`, `events.social_link`, `profiles` table |
| 4 | **QR Code Generator (Fan Engagement)** | - | - | - | **(Coming Soon)** Build your fanbase effortlessly from the stage. | **POST MVP** | | Future enhancement to Stage Plot |
| 5 | **Blog/Social Media Content Generator** | - | - | - | **(Future)** AI-powered blog and social post generator. Answer a few questions about your gig and get compelling content with a hook beyond just "time, place, band." | **POST MVP** | Help musicians promote gigs with engaging stories | Future Navigator/Promotion feature |
